...
{
    public partial class Scanin_report : Form
    {
        private int rowIndex = 0; // Declare rowIndex variable at the class level

        public Scanin_report()
        {
            InitializeComponent();
        }

        private void Scanin_report_Load(object sender, EventArgs e)
        {
        ...
        string query = "SELECT * FROM IT_INVENTORY WHERE STATUS = 'SCAN_IN' AND DATE BETWEEN @StartDate AND @EndDate";
